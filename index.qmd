...
#Reading in files
drwho2023<-read_json("drwho-72724.json", simplifyVector = T, simplifyDataFrame=F)
drwho2005<-read_json("drwho-210.json", simplifyVector = T, simplifyDataFrame=F)
drwho1963<-read_json("drwho-766.json", simplifyVector = T, simplifyDataFrame=F)

#To open and take a look at the files (made these comments because it took a long time to load)
#drwho2023
#drwho2005
#drwho1963
```
...
:::callout
It looks like the data is stored in a series of multi-level lists. When I open each file, I am able to click on each list index and see the information within it.
:::
...
readLines("drwho-210.json") |> prettify() |> writeLines(con ="drwho-210-pretty.json")
```
...
```{r}
#DrWho1963 Episode Info
episode_info<-tibble(id = drwho1963 |> map_dbl("id"), season = drwho1963 |> map_dbl("season"), number=drwho1963 |>map_int("number"), name = drwho1963 |> map_chr("name"), airtime= drwho1963 |> map_chr("airtime"))
head(episode_info,5)
```
